...
async function getAllFollowers(req,res){
    ...
    try{
        const result = await pool.query("SELECT following_user_id,username FROM backend.follows INNER JOIN backend.users ON following_user_id = backend.users.id WHERE followed_user_id = $1",[user_id])
...
async function getUserFollowing(req,res){
    ...
    try{
        const result = await pool.query("SELECT followed_user_id,username FROM backend.follows INNER JOIN backend.users ON followed_user_id = backend.users.id WHERE following_user_id = $1",[user_id])
        res.status(200).json(result.rows)
    }
    catch (err){
        console.log(err.message)
